...
!------ initialize component models ------
    ...
    if (file_exists('data_table') .and. .NOT.Atm%bounded_domain) then
      inquire(file='data_table', size=dt_size)
      if (dt_size > 0) then
        call data_override_init (Atm_domain_in  = Atm%domain)
      else
        call error_mesg ('program coupler', 'empty data table, skipping data override init', NOTE)
      endif
    else
      call error_mesg ('program coupler', 'no data table, skipping data override init', NOTE)
    endif
