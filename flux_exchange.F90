module flux_exchange_mod
  ...
  use mpp_mod,         only: mpp_npes, mpp_pe, mpp_error
...
!DIRECT: same grid, same decomp, direct copy
  use atmos_model_mod, only: atmos_boundary_data_type, land_ice_atmos_boundary_type
  ...
  use    land_model_mod, only:  land_boundary_data_type, atmos_land_boundary_type
...
!-----------------------------------------------------------------------
  character(len=128) :: version = '$Id: flux_exchange.F90,v 1.7 2002/02/22 19:07:19 fms Exp $'
...
!---- allocatable module storage ------
  ...
  real, allocatable, dimension(:) ::  ex_t_surf, ex_dhdt_surf, ex_dedt_surf, &
                                      ex_drdt_surf, ex_dhdt_atm,  ex_dedq_atm, &
...
!via field_manager
    ice_ocean_boundary%xtype = REDIST
    if( Ocean%domain.EQ.Ice%domain )ice_ocean_boundary%xtype = DIRECT
...
!                                        u_star_atm, b_star_atm
    ...
    real, dimension(n_xgrid_sfc) :: ex_albedo, ex_land_frac,  &
    ...
    logical, dimension(n_xgrid_sfc) :: ex_glacier, ex_land
...
!------ allocate storage also needed in flux_up_to_atmos -----
    ...
    allocate ( ex_t_surf   (n_xgrid_sfc),  &
    ...
    allocate ( ex_f_t_delt_n   (n_xgrid_sfc),  &
...
!---- put land quantities onto exchange grid ----
    ...
    call put_to_xgrid (Land%stomatal,   'LND', ex_stomatal,    xmap_sfc)
    call put_to_xgrid (Land%snow,       'LND', ex_snow,        xmap_sfc)
    call put_to_xgrid (Land%water,      'LND', ex_water,       xmap_sfc)
    call put_to_xgrid (Land%max_water,  'LND', ex_max_water,   xmap_sfc)

    ex_land_frac = 0.0
    call put_logical_to_real (Land%mask,    'LND', ex_land_frac, xmap_sfc)
    ex_glac_frac = 0.0
    call put_logical_to_real (Land%glacier, 'LND', ex_glac_frac, xmap_sfc)
...
!------- glacier fraction -----
        if ( id_glac_mask > 0 ) then
            call get_from_xgrid (diag_atm, 'ATM', ex_glac_frac, xmap_sfc);
